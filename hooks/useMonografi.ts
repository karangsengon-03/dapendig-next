...
async function fetchMonografi(): Promise<MonografiData> {
  ...
  const piramida = KELOMPOK_UMUR.map((k) => ({
    kelompok: k.label,
    laki: 0,
    perempuan: 0,
  }))
  ...
  for (const p of all) {
    ...
    // Pekerjaan (ambil top saja, skip yang 0)
    byPekerjaan[p.pekerjaan] = (byPekerjaan[p.pekerjaan] ?? 0) + 1

    // Status perkawinan
    byStatusPerkawinan[p.status_perkawinan] =
      (byStatusPerkawinan[p.status_perkawinan] ?? 0) + 1
    ...
    if (p.tanggal_lahir) {
      ...
      const idx = KELOMPOK_UMUR.findIndex(
        (k) => umur >= k.min && umur <= k.max
      )
      if (idx >= 0) {
