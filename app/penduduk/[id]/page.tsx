...
const STATUS_COLOR: Record<string, string> = {
  ...
  'tidak aktif': 'text-slate-400',
  'meninggal': 'text-rose-400',
  'mutasi-keluar': 'text-orange-400',
}
...
function hitungUmur(tanggalLahir: string): string {
  ...
  if (
    now.getMonth() - lahir.getMonth() < 0 ||
    (now.getMonth() - lahir.getMonth() === 0 && now.getDate() < lahir.getDate())
  ) umur--
  return `${umur} tahun`
}
  ...
  return (
    <AppShell title="Detail Penduduk">
          ...
          {!isLoading && data && (
            <div className="flex gap-2 flex-wrap justify-end">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm hover:bg-emerald-500/20 transition-colors"
              >
                <Printer size={13} />
                <span className="hidden sm:inline text-xs">Surat</span>
              </button>
              {canEdit && (
                <button
                  onClick={() => router.push(`/penduduk/${id}/edit`)}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-sm hover:bg-amber-500/20 transition-colors"
                >
                  <Pencil size={13} />
              ...
              {canEdit && isAktif && (
                <button
                  onClick={() => setShowCatatPindah(true)}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-sky-500/10 border border-sky-500/20 text-sky-400 text-sm hover:bg-sky-500/20 transition-colors"
                >
                  <LogOut size={13} />
              ...
              {canEdit && isAktif && (
                <button
                  onClick={() => setShowCatatMeninggal(true)}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-500/10 border border-slate-500/20 text-slate-400 text-sm hover:bg-slate-500/20 transition-colors"
                >
                  <HeartCrack size={13} />
                  <span className="hidden sm:inline text-xs">Meninggal</span>
                </button>
              )}
              {canDelete && (
                <button
                  onClick={() => setShowDelete(true)}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-sm hover:bg-rose-500/20 transition-colors"
                >
                  <Trash2 size={13} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <DetailRow label="No. KK">
                {isAktif ? (
                  <button
                    ...
                    className="text-sky-400 underline underline-offset-2 hover:text-sky-300 transition-colors text-sm"
                  >
                    {data.no_kk || '—'}
                  </button>
                ) : (
            ...
            <div className="rounded-2xl border border-white/[0.06] bg-[#0d1424] px-4 py-1">
              ...
              <DetailRow label="Tempat, Tanggal Lahir">
                {`${data.tempat_lahir}, ${new Date(data.tanggal_lahir + 'T00:00:00').toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}`}
              </DetailRow>
      ...
      {showCatatPindah && data && (
        <CatatPindahKeluarModal
          penduduk={data}
          onClose={() => setShowCatatPindah(false)}
